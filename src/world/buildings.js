...
import { C, mix, shade } from '../core/palette.js'
...
const WALL_TONES = [C.stone, C.stoneMid, C.stoneLit]
const DARK_TONES = [C.stoneDeep, C.stoneDark, C.stoneMid]
...
export function homestead(tier = 1) {
  ...
  parts.push({ geometry: chamferBox(w + 0.5, 0.28, d + 0.5, 0.07), position: [0, 0.14, 0], color: C.stoneDark })
  walls(parts, { w, d, h, doorW: 1.2 })

  // Doorway: the lune arch, recessed, with a dark reveal behind it.
  parts.push({ geometry: luneArch(1.2, h * 0.62, 0.1), position: [0, 0.28, d / 2 + 0.06], color: C.stoneShadow })
  parts.push({ geometry: luneArch(1.42, h * 0.68, 0.14), position: [0, 0.24, d / 2 - 0.02], color: C.stoneLit })
  // The lintel stone, with the mark cut into it.
  parts.push({ geometry: chamferBox(1.7, 0.34, 0.34, 0.06), position: [0, h * 0.68 + 0.34, d / 2], color: C.stonePale })

  // Roof: a wedge, and a ridge beam that overhangs. The overhang is what stops
  // it reading as a lid.
  parts.push({ geometry: wedge(w + 0.7, 1.15, d + 0.7), position: [0, h, 0], color: C.stoneDeep })
  parts.push({ geometry: chamferBox(w + 0.9, 0.14, 0.18, 0.04), position: [0, h + 1.16, 0], color: C.stoneDark })
  parts.push({ geometry: chamferBox(w + 0.85, 0.12, 0.16, 0.035), position: [0, h + 0.06, d / 2 + 0.36], color: C.creamShade })
  parts.push({ geometry: chamferBox(w + 0.85, 0.12, 0.16, 0.035), position: [0, h + 0.06, -d / 2 - 0.36], color: C.creamShade })

  // Windows, as lune arches in cream — the only cream on the building, so they
  // are what your eye finds from across the valley.
  for (const side of [-1, 1]) {
    parts.push({ geometry: luneArch(0.52, 0.8, 0.1), position: [side * (w / 2 + 0.02), h * 0.28, 0], rotation: [0, side * Math.PI / 2, 0], color: C.creamDeep })
  }
  ...
  if (t >= 2) {
    ...
    parts.push({ geometry: chamferBox(w - 0.5, 0.16, 1.5, 0.05), position: [0, 2.16, d / 2 + 0.6], color: C.stoneDeep })
    parts.push({ geometry: chamferBox(w + 0.2, 0.2, 0.2, 0.05), position: [0, 0.2, d / 2 + 1.3], color: C.stoneDark })
  }
  if (t >= 3) {
    // Flue, for the kiln that got built into the house.
    parts.push({ geometry: chamferBox(0.62, 1.5, 0.62, 0.08), position: [w / 2 - 0.9, h + 0.9, -d / 4], color: C.stoneMid })
    parts.push({ geometry: chamferBox(0.78, 0.18, 0.78, 0.05), position: [w / 2 - 0.9, h + 1.72, -d / 4], color: C.stoneDark })
  }
  if (t >= 4) {
    // The tower and its shard lantern — visible from the ridge, which is the
    // point of it.
    parts.push({ geometry: chamferBox(1.5, 2.4, 1.5, 0.1), position: [-w / 2 + 0.7, h + 1.2, -d / 4], color: C.stone })
    parts.push({ geometry: FLARE, position: [-w / 2 + 0.7, h + 2.55, -d / 4], scale: [1.9, 0.44, 1.9], color: C.stoneDeep })
    parts.push({ geometry: shardGeometry(), position: [-w / 2 + 0.7, h + 3.1, -d / 4], scale: [0.42, 0.55, 0.34], color: C.rose })
  }
...
export function ridgeGate() {
  const parts = []
  for (const side of [-1, 1]) {
    parts.push({ geometry: chamferBox(1.1, 4.6, 1.1, 0.14), position: [side * 2.0, 2.3, 0], color: C.stoneLit })
    parts.push({ geometry: chamferBox(1.35, 0.4, 1.35, 0.09), position: [side * 2.0, 0.2, 0], color: C.stoneDark })
    parts.push({ geometry: chamferBox(1.3, 0.36, 1.3, 0.08), position: [side * 2.0, 4.66, 0], color: C.stonePale })
    // The half-lune: a crescent of stone leaning in toward its twin.
    parts.push({ geometry: TAPER, position: [side * 1.45, 3.3, 0], scale: [0.55, 2.6, 0.9], color: C.stonePale })
  }
  // The lintel, and the mark hung under it.
  parts.push({ geometry: chamferBox(5.4, 0.5, 1.0, 0.1), position: [0, 5.1, 0], color: C.stoneDeep })
  parts.push({ geometry: chamferBox(4.4, 0.34, 0.5, 0.07), position: [0, 4.66, 0.3], color: C.stoneShadow })
  parts.push({ geometry: markFlatGeometry(), position: [0, 4.66, 0.58], scale: [1.6, 1.6, 1], color: C.creamDeep })
  // Steps up to it.
  for (let i = 0; i < 3; i++) {
    parts.push({ geometry: chamferBox(5.2 - i * 0.5, 0.22, 1.0, 0.05), position: [0, 0.11 + i * 0.2, 2.2 - i * 0.7], color: i % 2 ? C.stoneMid : C.stone })
  }
...
export function kiln() {
  const parts = [
    { geometry: chamferBox(2.4, 0.36, 2.4, 0.08), position: [0, 0.18, 0], color: C.stoneDark },
    { geometry: FLARE, position: [0, 1.1, 0], scale: [2.2, 1.9, 2.2], color: C.stone },
    { geometry: TAPER, position: [0, 2.4, 0], scale: [1.5, 1.0, 1.5], color: C.stoneMid },
    { geometry: FLAT, position: [0, 2.95, 0], scale: [1.0, 0.2, 1.0], color: C.stoneDark },
    { geometry: luneArch(0.8, 1.0, 0.3), position: [0, 0.32, 0.72], color: C.stoneShadow },
    { geometry: chamferBox(1.2, 0.22, 0.3, 0.05), position: [0, 1.4, 0.78], color: C.stonePale },
  ]
...
export function shed() {
  const parts = [
    { geometry: chamferBox(4.6, 0.24, 3.4, 0.06), position: [0, 0.12, 0], color: C.stoneDark },
  ]
  for (const sx of [-1, 1]) {
    for (const sz of [-1, 1]) {
  ...
  parts.push({ geometry: wedge(5.4, 0.9, 4.0), position: [0, 2.3, 0], color: C.stoneDeep })
  parts.push({ geometry: chamferBox(5.6, 0.12, 0.16, 0.035), position: [0, 2.34, 2.0], color: C.creamShade })
  // The stock: three lumps of cut stone, which is what tells you what the shed
  // is for without a sign on it.
  parts.push({ geometry: stoneLump(3, { radius: 0.5, height: 0.7 }), position: [-1.1, 0.47, -0.5], color: C.stoneMid })
  parts.push({ geometry: stoneLump(9, { radius: 0.42, height: 0.6 }), position: [-0.4, 0.42, -0.9], color: C.stone })
  parts.push({ geometry: chamferBox(1.1, 0.5, 0.8, 0.06), position: [1.2, 0.37, -0.4], color: C.stoneLit })
...
export function vault() {
  const parts = [
    { geometry: chamferBox(3.8, 0.34, 3.4, 0.08), position: [0, 0.17, 0], color: C.stoneDark },
    { geometry: chamferBox(3.2, 1.5, 2.8, 0.12), position: [0, 1.05, 0], color: C.stone },
    { geometry: TAPER, position: [0, 2.0, 0], scale: [3.0, 0.9, 2.7], color: C.stoneMid },
    { geometry: FLAT, position: [0, 2.5, 0], scale: [1.2, 0.24, 1.2], color: C.stonePale },
    { geometry: luneArch(0.9, 1.3, 0.24), position: [0, 0.34, 1.42], color: C.stoneShadow },
    { geometry: chamferBox(1.3, 0.24, 0.3, 0.05), position: [0, 1.72, 1.46], color: C.stoneLit },
    { geometry: shardGeometry(), position: [0, 1.98, 1.5], scale: [0.3, 0.4, 0.22], color: C.rose },
  ]
...
export function cairn(level = 1) {
  ...
  const parts = [{ geometry: chamferBox(1.5, 0.2, 1.5, 0.05), position: [0, 0.1, 0], color: C.stoneDark }]
  let y = 0.2
  for (let i = 0; i < n; i++) {
    const t = i / n
    const s = 1.15 - t * 0.55
    ...
    parts.push({
      ...
      color: [C.stone, C.stoneLit, C.stoneMid, C.stoneDeep][i % 4],
    })
    y += hgt
  }
  parts.push({ geometry: FLAT, position: [0, y + 0.06, 0], scale: [0.5, 0.12, 0.5], color: C.stonePale })
...
export function crate() {
  const parts = [
    ...
    { geometry: chamferBox(1.78, 0.16, 1.28, 0.04), position: [0, 0.98, 0], color: C.stoneDeep },
    { geometry: chamferBox(1.78, 0.16, 1.28, 0.04), position: [0, 0.12, 0], color: C.stoneDeep },
    { geometry: chamferBox(0.16, 1.04, 1.28, 0.04), position: [-0.85, 0.5, 0], color: C.stoneDark },
    { geometry: chamferBox(0.16, 1.04, 1.28, 0.04), position: [0.85, 0.5, 0], color: C.stoneDark },
    { geometry: markFlatGeometry(), position: [0, 0.56, 0.61], scale: [0.62, 0.62, 1], color: C.creamShade },
  ]
...
export function well() {
  const parts = [
    { geometry: COLUMN, position: [0, 0.42, 0], scale: [1.8, 0.84, 1.8], color: C.stone },
    { geometry: COLUMN, position: [0, 0.86, 0], scale: [1.5, 0.1, 1.5], color: C.stoneShadow },
    { geometry: COLUMN, position: [0, 0.88, 0], scale: [1.86, 0.12, 1.86], color: C.stoneLit },
  ]
  for (const side of [-1, 1]) {
    parts.push({ geometry: COLUMN, position: [side * 0.7, 1.5, 0], scale: [0.16, 1.4, 0.16], color: C.trunk })
  }
  parts.push({ geometry: chamferBox(2.0, 0.14, 0.5, 0.04), position: [0, 2.24, 0], color: C.stoneDeep })
  parts.push({ geometry: ROD, position: [0, 2.1, 0], scale: [0.1, 1.5, 0.1], rotation: [0, 0, Math.PI / 2], color: C.stoneDark })
...
export function flagpole() {
  ...
  const mast = bake([
    { geometry: COLUMN, position: [0, 2.6, 0], scale: [0.13, 5.2, 0.13], color: C.stoneDeep },
    { geometry: chamferBox(0.6, 0.24, 0.6, 0.06), position: [0, 0.12, 0], color: C.stoneDark },
    { geometry: POINT, position: [0, 5.34, 0], scale: [0.26, 0.34, 0.26], color: C.stonePale },
  ])
  ...
  const flag = new THREE.Mesh(
    cloth,
    new THREE.MeshLambertMaterial({ map: markTexture({ ink: C.cream, ground: C.ink, size: 256, scale: 0.62 }), side: THREE.DoubleSide }),
  )
...
export const MASONRY = { light: WALL_TONES, dark: DARK_TONES, mortar: mix(C.stoneDark, C.ink, 0.4), shadow: shade(C.stone, 1) }
